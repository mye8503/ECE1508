...
def get_action(current_day_idx, data, target_weight): 
    close = data['Close'] 
    current_day = data.index[current_day_idx]

    num_shares = target_weight / close[current_day:current_day]
    num_shares = num_shares.values.flatten()
    new_allocation = num_shares / LA.norm(num_shares, 1)
...
if __name__ == "__main__":
    ...
    for i in range(len(testset_paths)):
        ...
        portfolio_vals = []
        while not done:
            action = get_action(current_day_idx, data, target_weight)
